//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Mar 11 2021 20:53:35).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <Flexo/FFImageRep.h>

#import <Flexo/FFFlatImageRepWithPlanes-Protocol.h>

@class FFImage, FFScheduleTokenVTDecode, FFStreamVideoFig;

__attribute__((visibility("hidden")))
@interface FFImageRepFigSchedFrame : FFImageRep <FFFlatImageRepWithPlanes>
{
    FFStreamVideoFig *_stream;
    FFScheduleTokenVTDecode *_schedRec;
    FFImage *_finalImage;
    int _lineSel;
    struct CGRect _origBounds;
    struct CGRect _afterDecodeBounds;
    FFImage *_fillerImage;
    BOOL _disable422Filtering;
}

- (_Bool)imageReadyWithoutBlocking;
- (int)errorCode;
- (void)getSampleBuffer:(struct opaqueCMSampleBuffer **)arg1 alphaKnownToBeFullyOpaque:(char *)arg2;
- (struct HGBitmap *)copyHGBitmapForPlane:(int)arg1;
- (int)numPlanes;
- (id)newTextureBufferWithFormatByFlatteningIfNeeded:(id)arg1 location:(struct FxDevice *)arg2 roi:(struct CGRect)arg3 pixelTransform:(id)arg4 flattenOptions:(const CDStruct_f51ed29d *)arg5 workingSpace:(int)arg6;
- (id)newPixelBufferWithFormatByFlatteningIfNeeded:(id)arg1 roi:(struct CGRect)arg2 pixelTransform:(id)arg3 nonStandardAlphaOK:(_Bool)arg4 flattenOptions:(const CDStruct_f51ed29d *)arg5 getRawPixelsWithoutSourceConform:(_Bool)arg6 workingSpace:(int)arg7;
- (id)_fillerFrame;
- (int)getConversionTypeForPF:(id)arg1 colorSpace:(struct CGColorSpace *)arg2 pixelTransform:(id)arg3 location:(struct FxDevice *)arg4 nonStandardAlphaOK:(_Bool)arg5 backingRequest:(int)arg6;
- (unsigned long long)approximateMemoryUsage;
- (struct CGColorSpace *)colorSpace;
- (struct CGColorSpace *)_internalColorSpace:(_Bool)arg1;
- (id)nativePixelFormat;
- (_Bool)alphaWantsClamping;
- (_Bool)alphaWantsInversion;
- (int)nativeAlphaType;
- (id)pixelTransform;
- (id)_internalPixelTransform:(_Bool)arg1;
- (struct CGRect)pixelSpaceBounds;
- (void)_ensureImageCharacteristics;
- (void)_ensureFinalImage;
- (id)copyFinalImage;
- (id)type;
- (id)description;
- (void)dealloc;
- (id)initWithPixelTransform:(id)arg1 stream:(id)arg2 scheduleRecord:(id)arg3 lineSel:(int)arg4 pixelSpaceBounds:(struct CGRect)arg5 field:(unsigned int)arg6 disable422Filtering:(BOOL)arg7 sourceColorConformer:(id)arg8 sourceCCTargetColorSpace:(int)arg9;

@end

