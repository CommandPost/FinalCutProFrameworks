//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "POOnScreenControl.h"

#import "OZObjCObserver.h"

@class TXGlyphScale3D;

@interface TXGlyphOSC : POOnScreenControl <OZObjCObserver>
{
    double _startX;
    double _startY;
    double _mouseX;
    double _mouseY;
    double _startViewX;
    double _startViewY;
    double _mouseViewX;
    double _mouseViewY;
    double _snapStartX;
    double _snapStartY;
    double _anchorX;
    double _anchorY;
    double _anchorZ;
    double _pivotAnchorX;
    double _pivotAnchorY;
    double _pivotAnchorZ;
    struct map<TXTextObject *, PCMatrix44Tmpl<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCMatrix44Tmpl<double>>>> *_textObjectStartTransformMatrix;
    struct map<TXTextObject *, PCVector3<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCVector3<double>>>> *_startAnchor;
    struct map<TXTextObject *, PCVector3<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCVector3<double>>>> *_startPivotAnchor;
    struct map<TXTextObject *, PCRect<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCRect<double>>>> *_textObjectStartBounds;
    struct map<TXTextObject *, PCBox<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCBox<double>>>> *_textObjectStartBox;
    struct map<TXTextObject *, PCVector3<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCVector3<double>>>> *_startScale;
    struct map<TXTextObject *, PCVector3<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCVector3<double>>>> *_startRotation;
    struct map<TXTextObject *, PCVector3<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCVector3<double>>>> *_startOffset;
    struct map<TXTextObject *, PCVector3<double>, std::__1::less<TXTextObject *>, std::__1::allocator<std::__1::pair<TXTextObject *const, PCVector3<double>>>> *_immediateOffset;
    double _startScaleX;
    double _startScaleY;
    double _startScaleZ;
    double _startRotX;
    double _cumulativeRotX;
    double _lastRotX;
    double _startRotY;
    double _cumulativeRotY;
    double _lastRotY;
    double _startRotZ;
    double _cumulativeRotZ;
    double _lastRotZ;
    double _startTransX;
    double _startTransY;
    double _startTransZ;
    double _startPivotX;
    double _startPivotY;
    double _startPivotZ;
    double _startShearX;
    double _startShearY;
    PCRect_b601f9f3 *_startBounds;
    PCBox_dd92ab54 *_startBox;
    PCMatrix44Tmpl_93ed1289 *_thisTextObjectStartTransformMatrix;
    double _draggingScaleX;
    double _draggingScaleY;
    double _draggingScaleZ;
    double _draggingRotX;
    double _draggingRotY;
    double _draggingRotZ;
    double _draggingTransX;
    double _draggingTransY;
    double _draggingTransZ;
    double _draggingPivotX;
    double _draggingPivotY;
    double _draggingPivotZ;
    BOOL _moving;
    BOOL _rotating;
    BOOL _3dtranslating;
    BOOL _movingObject;
    BOOL _boxSelect;
    BOOL _toggleBoxSelect;
    struct TXDiscontinuousSelection _glyphSelection;
    unsigned long long _focusedGlyphSelection;
    BOOL _cmdKeyDown;
    BOOL _shiftKeyDown;
    BOOL _altKeyDown;
    BOOL _isChanDelta;
    _Bool _mouseIsDown;
    unsigned long long _curCharOffset;
    int _mActivePart;
    double _startCross;
    _Bool _capturingModifiedChannels;
    struct POMove3DOSC *_pos3Dtool;
    struct PORotate3DOSC *_rot3Dtool;
    TXGlyphScale3D *_scale3D;
    PCVector3_457fd1f0 _pAxis;
    PCVector3_457fd1f0 _OSCStartRot;
    PCVector3_457fd1f0 _OSCStartTrans;
    PCVector2_79efa81a _OSCStartScale;
    BOOL _firstDrag;
}

+ (BOOL)validate:(struct OZChannelBase *)arg1;
- (id).cxx_construct;
- (void).cxx_destruct;
- (BOOL)isMoving;
- (void)setIsMoving:(BOOL)arg1;
- (BOOL)scalePositionByFontSize;
- (struct PCUUID)getClassID;
- (void)clipPoints:(struct TXTextObject *)arg1 camera:(const struct LiCamera *)arg2 state:(const struct OZRenderState *)arg3 points:(const vector_ced1dec3 *)arg4 visible:(vector_a7cf9eda *)arg5;
- (void)clipOutline:(struct TXTextObject *)arg1 camera:(const struct LiCamera *)arg2 state:(const struct OZRenderState *)arg3 points:(const PCVector2_79efa81a *)arg4 numPts:(int)arg5 edges:(PCVector2_79efa81a *)arg6 visible:(vector_a7cf9eda *)arg7;
- (int)getOSCMode;
- (BOOL)isGlyphOSC;
- (BOOL)useCurrentSequenceValues;
- (int)getTransformScope;
- (struct OZRenderState)getRenderState:(const struct OZRenderState *)arg1 textObject:(PCPtr_3bd68a10)arg2 remapTime:(BOOL)arg3;
- (BOOL)drawMultipleSelectionBoxes;
- (void)fourCornerWillChange;
- (void)rotationWillChange;
- (void)scaleWillChange;
- (void)offsetWillChange;
- (unsigned int)getGlyphSelectionSize;
- (_Bool)scaleAffectsOffset;
- (_Bool)scaleAffectsLayout;
- (void)notify:(unsigned int)arg1;
- (id)menuForEvent:(id)arg1;
- (struct TXGlyphTransformInfo)getGlyphTransformInfo:(PCPtr_3bd68a10)arg1 forRenderState:(struct OZRenderState *)arg2;
- (BOOL)postRedisplayOnActivePartChange;
- (struct OZElement *)getElement;
- (void)setMActivePart:(int)arg1;
- (int)hitCheck:(id)arg1;
- (BOOL)selectNone:(id)arg1;
- (BOOL)selectAll:(id)arg1;
- (BOOL)updateSelectionInOSC:(id)arg1 withRect:(PCRect_b601f9f3 *)arg2;
- (void)updateSelection:(id)arg1;
- (void)initSelectionInOSC:(id)arg1;
- (void)updateRotation:(id)arg1 change:(const PCQuat_de30948b *)arg2;
- (BOOL)oscFlagsChanged:(id)arg1;
- (BOOL)keyDownOSC:(id)arg1;
- (void)mouseUp:(id)arg1;
- (void)mouseDragged:(id)arg1;
- (void)mouseDown:(id)arg1;
- (void)snapshotState:(id)arg1;
- (void)rotate:(id)arg1 angle:(double)arg2;
- (void)translate:(id)arg1 hitPart:(int)arg2 translateX:(double)arg3 translateY:(double)arg4 translateZ:(double)arg5 state:(struct OZRenderState)arg6;
- (void)scale:(id)arg1 hitPart:(int)arg2 scaleX:(double)arg3 scaleY:(double)arg4 scaleZ:(double)arg5;
- (void)warp:(id)arg1 hitPart:(int)arg2 deltaX:(double)arg3 deltaY:(double)arg4;
- (void)getTransformMatrix:(PCMatrix44Tmpl_93ed1289 *)arg1 textObject:(struct TXTextObject *)arg2 renderParams:(struct OZRenderParams *)arg3 withCharScale:(_Bool)arg4;
- (PCBox_dd92ab54 *)startBox:(struct TXTextObject *)arg1;
- (PCVector3_457fd1f0 *)startAnchor:(struct TXTextObject *)arg1;
- (PCVector3_457fd1f0 *)startPivot:(struct TXTextObject *)arg1;
- (PCMatrix44Tmpl_93ed1289 *)startTransformMatrix:(struct TXTextObject *)arg1;
- (void)getAnchorX:(double *)arg1 andAnchorY:(double *)arg2 andAnchorZ:(double *)arg3 forTextObject:(struct TXTextObject *)arg4 forFixedAnchor:(BOOL)arg5;
- (struct OZChannelQuad *)getFourCornerChannel:(const shared_ptr_e0e110e1 *)arg1 forAttribute:(int)arg2;
- (struct OZChannelRotation3D *)getRotationChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelScale3D *)getScaleChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelScale *)getDropShadowScaleChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelAngle *)getDropShadowAngleChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelDouble *)getDropShadowDistanceChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelPosition *)getGlowOffsetChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelScale *)getGlowScaleChannel:(const shared_ptr_e0e110e1 *)arg1;
- (struct OZChannelPosition3D *)getOffsetChannel:(const shared_ptr_e0e110e1 *)arg1;
- (PCVector3_457fd1f0)getDraggingAxis;
- (PCMatrix44Tmpl_93ed1289)getWorldToViewMatrix;
- (void)getLocalToAxesOrientation:(const PCMatrix44Tmpl_93ed1289 *)arg1 localToAxesOrientation:(PCMatrix44Tmpl_93ed1289 *)arg2 withState:(const struct OZRenderState *)arg3;
- (void)getAxesToWorld:(struct TXTextObject *)arg1 axesToWorld:(PCMatrix44Tmpl_93ed1289 *)arg2 withState:(const struct OZRenderState *)arg3;
- (void)getCameraToWorldRotation:(PCMatrix44Tmpl_93ed1289 *)arg1;
- (void)updateInfoBarWithCornerScale:(const PCVector2_79efa81a *)arg1 andChange:(const PCVector2_79efa81a *)arg2;
- (void)updateInfoBarWithSideScale:(float)arg1 andChange:(float)arg2;
- (void)updateInfoBarWithRotation:(const PCVector3_457fd1f0 *)arg1 andChange:(const PCVector3_457fd1f0 *)arg2;
- (void)updateInfoBarWithPosition:(const PCVector3_457fd1f0 *)arg1 andChange:(const PCVector3_457fd1f0 *)arg2;
- (void)drawBoxSelection;
- (void)draw;
- (void)dealloc;
- (id)initWithHostDelegate:(id)arg1 andViewDelegate:(id)arg2 andObjectDelegate:(id)arg3 andChannel:(struct OZChannelBase *)arg4;

@end

